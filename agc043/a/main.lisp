...
(defun main ()
  ...
  (let ((h (read))
        (w (read)))
    (let ((matrix (read-characters-to-board h w))
          (memo (make-hash-table :test #'eql)))
      (labels ((encode (y x)
                 (+ y (* x 110)))
               (dfs (y x)
                 (let ((val (gethash (encode y x) memo)))
                   (cond
                     (val val)
                     ((and (= y (1- h))
                           (= x (1- w)))
                      0)
                     (t (setf (gethash (encode y x) memo)
                              (reduce #'min
                                      (mapcar (lambda (d)
                                                (let ((ny (+ y (first d)))
                                                      (nx (+ x (rest d))))
                                                  (+ (dfs ny nx)
                                                     (if (and (zerop (aref matrix y x))
                                                              (not (zerop (aref matrix ny nx))))
                                                         1
                                                         0))))
                                              (remove-if (lambda (d)
                                                           (let ((ny (+ y (first d)))
