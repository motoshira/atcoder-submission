...
(defun make-permutations (n)
  (let ((res nil))
    (nlet rec ((xs (loop for i from 1 to n collect i))
               (acc nil))
      (cond
        ((null xs) (push (reverse acc) res))
        (t (dolist (x xs)
...
(defun main ()
  ...
  (let* ((n (read))
         ...
         (qs (loop repeat n collect (read)))
         (lsts (make-permutations n)))
    (println (abs (- (position qs lsts :test #'equal)
                     (position ps lsts :test #'equal))))))
