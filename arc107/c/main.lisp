...
(defun matrix-rotate (matrix)
  (let ((n (isqrt (array-total-size matrix))))
                   ...
                   (mapc (lambda (i1)
                           (mapc (lambda (i2)
                                   (when (every (lambda (j)
                                                ...
                                                (iota n))
                                     (uf-unite uf i1 i2)))
                                 (loop for i2 from (1+ i1) below n collect i2)))
                         (iota n))
                   (reduce #'mod* (mapcar (lambda (i)
                                            (if (= (uf-find uf i)
                                                   i)
                                                (mod-fact (uf-get-tree-size uf i))
                                                1))
                                          (iota n))
                           :initial-value 1))))
        (println (mod* (row-inspect matrix)
                       (row-inspect (matrix-rotate matrix))))))))
