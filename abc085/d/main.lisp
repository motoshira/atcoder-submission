...
(defun main ()
  ...
  (let* ((n (read))
         (hp (read))
         (lst (loop repeat n collect (cons (read-fixnum)
                                           (read-fixnum))))
         (a-max (reduce #'max (mapcar #'first lst)))
         (bs (sort (mapcar #'rest lst) #'>)))
    (declare (fixnum n hp a-max)
             (list lst bs))
    (println (named-let rec ((hp hp)
                             (bs bs)
                             (cnt 0))
               (declare (fixnum hp cnt)
                        (list bs))
               (cond
                 ((<= hp 0) cnt)
                 ((or (null bs)
                      (<= (the fixnum (first bs)) a-max))
                  (the fixnum (+ cnt (ceiling hp a-max))))
                 (t (rec (the fixnum (- hp
                                        (the fixnum (first bs))))
                         (rest bs)
                         (1+ cnt))))))))
