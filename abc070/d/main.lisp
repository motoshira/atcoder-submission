...
(defun main ()
  ...
  (let ((n (read-fixnum)))
    ...
    (let ((edges (make-array n
      ...
      (dotimes (_ (1- n))
        (declare (ignorable _))
        (multiple-value-bind (a b c) (values (1- (read-fixnum))
                                             (1- (read-fixnum))
                                             (read-bignum))
          (declare (fixnum a b)
                   (integer c))
          (push (list a c) (aref edges b))
          (push (list b c) (aref edges a))))
      (let* ((q (read-fixnum))
             (k (1- (read-fixnum)))
             (dist-from-k (make-array n
                                      :element-type 'integer
                                      :adjustable nil
                                      :initial-element *inf*))
             (queries (loop repeat q collect (list (1- (read-fixnum))
                                                   (1- (read-fixnum))))))
        (declare (fixnum q k)
                 ((array integer 1) dist-from-k)
                 (list queries))
        (named-let rec ((node k)
                        (parent -1)
                        (d 0))
          (declare (fixnum node parent)
                   (integer d))
          (unless (< (the integer (aref dist-from-k node))
                     *inf*)
               (setf (aref dist-from-k node) d)
               (mapc (lambda (list)
                       (destructuring-bind (next dist) list
                         (declare (fixnum next)
                                  (integer dist))
                         (unless (= next parent)
                           (rec next
                                ...
                                (+ d dist)))))
                     (aref edges node))))
        (princ-for-each-line (mapcar (lambda (query)
                                       (destructuring-bind (x y) query
                                         (declare (fixnum x y))
                                         (+ (aref dist-from-k x)
                                            (aref dist-from-k y))))
                                     queries))))))
