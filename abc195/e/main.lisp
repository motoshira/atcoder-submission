...
(defun main ()
  (declare #.*opt*)
  (let ((n (read))
        (xs (map '(simple-array fixnum 1) #'digit-char-p (read-line)))
        (s (read-line))
        (memo (make-hash-table :test #'eq)))
    (declare (fixnum n)
             ((simple-array fixnum 1) xs)
             (string s)
             (hash-table memo))
    (labels ((encode (x y)
               (declare (fixnum x y))
               (the fixnum
                    (+ (* x 10)
                       y)))
             (rec (i r)
               (multiple-value-bind (val win) (gethash (encode i r) memo)
                 (when win
                   (return-from rec val)))
               (setf (gethash (encode i r) memo)
                     (cond
                       ((= i n)
                        (zerop r))
                       ((char= (char s i)
