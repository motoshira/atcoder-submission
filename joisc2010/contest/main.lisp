...
(defun main ()
  ...
  (let* ((n (read))
         ...
         (x (read))
         (y (read))
         (scores (make-array m :element-type 'fixnum))
         (counter (make-array n :element-type 'fixnum :initial-element 0))
    ...
    (do-rep y
      (let ((time (read))
            ...
            (problem-id (1- (read)))
            (result (read)))
        (dbg time person-id problem-id result)
        (ecase result
          (:open (setf (gethash (list person-id problem-id) logs) (list 0 time)))
          (:incorrect (incf (first (gethash (list person-id problem-id)
                                            logs))))
          (:correct (destructuring-bind (cnt start-time)
                        (gethash (list person-id problem-id)
                                 logs)
                      (incf (aref counter person-id)
                            (max x
                                 (- (aref scores problem-id)
                                    (- time start-time)
                                    (* 120 cnt)))))))))
    (dotimes (i n)
      (println (aref counter i)))))
