...
(defun main ()
  ...
  (let ((n (read))
    ...
    (let ((matrix (make-array (list n n)
                              ...
                              :initial-element nil)))
      (dotimes (i n)
        (setf (aref matrix i i) t))
      (dotimes (_ m)
        (let ((a (1- (read)))
              (b (1- (read))))
          (setf (aref matrix a b) t)
          (setf (aref matrix b a) t)))
      (println (named-let rec ((xs (iota n))
                               (acc nil))
                 (cond
                   ((null xs) (if (every (lambda (x)
                                           (every (lambda (y)
                                                    (aref matrix x y))
                                                  acc))
                                         acc)
                                  (length acc)
                                  -1))
                   (t (max (rec (rest xs) acc)
                           (rec (rest xs) (cons (first xs)
                                                acc))))))))))
