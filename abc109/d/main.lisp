...
(eval-when (:compile-toplevel :load-toplevel :execute)
  #+swank (progn (ql:quickload '(:sb-concurrency :fiveam :cl-debug-print :sb-sprof) :slient t))
...
(defun main ()
  ...
  (let ((h (read))
        (w (read)))
    (let ((matrix (make-array (list h w)
                              :element-type 'uint4
                              :adjustable nil)))
      (dotimes (i h)
        (dotimes (j w)
          (setf (aref matrix i j) (read-fixnum))))
      (labels ((get-next-point (y x)
                 (if (evenp y)
                     (if (= x (1- w))
                         (values (1+ y) x)
                         (values y (1+ x)))
               ...
               (proc (y x cnt acc)
                 (if (= cnt (1- (* h w)))
                     (reverse acc)
                     (multiple-value-bind (ny nx) (get-next-point y x)
                       (if (oddp (aref matrix y x))
                           (progn
                             (decf (aref matrix y x))
                             (incf (aref matrix ny nx))
                             (proc ny nx (1+ cnt) (cons (format nil "~a ~a ~a ~a" (1+ y) (1+ x) (1+ ny) (1+ nx))
                                                        acc)))
                           (proc ny nx (1+ cnt) acc))))))
        (let ((ans (proc 0 0 0 nil)))
          (println (length ans))
