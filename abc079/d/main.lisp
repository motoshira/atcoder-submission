...
(defun main ()
  ...
  (tlet ((h (read) uint8)
         (w (read) uint8))
    (tlet ((c-board (make-array '(10 10)
                                :adjustable nil
                                :element-type 'uint16)
                    (array uint16 2))
           (n-board (make-array `(,h ,w)
                                :adjustable nil
                                :element-type 'int16)
                    (array int16 2)))
      (dotimes (i 10)
        (dotimes (j 10)
          (setf (aref c-board i j) (read-fixnum))))
      (dotimes (i h)
        (dotimes (j w)
          (setf (aref n-board i j) (read-fixnum))))
      ;; Warshall-Floyd Algorithm
      (dotimes (i 10)
        (dotimes (j 10)
          (dotimes (k 10)
            (minf (aref c-board i j)
                  (+ (aref c-board i k)
                     (aref c-board k j))))))
      (println (ntlet (rec uint64) ((pos 0 uint16)
                                    (acc 0 uint64))
