...
(defun main () 
  ...
  (let ((n (read))
        (m (read)))
    (let* ((height (->> (loop repeat n collect (read-fixnum)) (sort :@ #'<) (apply #'vector)))
           (height-rev (reverse height))
           (w (loop repeat m collect (read-fixnum)))
           (l-cum (make-cum height))
           (r-cum (nreverse (make-cum height-rev)))
           (n/2 (floor n 2)))
      (println (reduce #'min
                       (mapcar (lambda (x)
                                 (named-let bs ((lo -1)
                                                (hi n))
                                   ;; heightの中でxより小さいもののmax-indexを探す
                                   (if (<= (abs (- lo hi))
                                           1)
                                       (+ (aref l-cum (ceiling lo 2))
                                          (if (evenp lo)
                                              (abs (- x
                                                      (aref height lo)))
                                              (abs (- x
                                                      (aref height hi))))
                                          (aref r-cum (ceiling lo 2)))
                                       (let ((mid (floor (+ lo hi)
                                                         2)))
                                         (if (< (aref height mid)
                                                x)
